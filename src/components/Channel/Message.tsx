...
const Reply = ({ parentId }: { parentId: number }) => {
  ...
  const parent = utils.messages.getMessagesByChannelId
    .getInfiniteData({
      channelId,
    })
  ...
  if (!parent) throw new Error("there must be a parent");
  console.log(parent);

  return (
    <div className="relative flex items-center overflow-hidden overflow-ellipsis whitespace-nowrap break-words pl-[72px] text-sm text-gray-400">
